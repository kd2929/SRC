           ...
           # medi =  msg.document or msg.video or msg.audio or None
            ...
            if str(file).split(".")[-1] in ['mkv', 'mp4', 'webm', 'mpe4', 'mpeg']:
                ...
                await client.send_video(
                    chat_id=sender,
                    video=path,
                    caption=caption.replace("@ImTgLoki", "**ＡＳＵＲ**\n`@AsurTalkBot`").replace("@LokiTheRobot", "**ＡＳＵＲ**\n`@AsurTalkBot`").replace("@TgLokii", "**ＡＳＵＲ**\n`@AsurTalkBot`").replace("by♛ऋषभ राजपूत","").replace("ℍ𝕖𝕝𝕡 𝕠𝕥𝕙𝕖𝕣𝕤 𝔾𝕠𝕕 𝕨𝕚𝕝𝕝 𝕙𝕖𝕝𝕡 𝕐𝕠𝕦", "").replace("RESOLUTION:-720", "").replace("Download By : 👑 KING R & 👸QUEEN P", "").replace("BATCH NAME :-" ,"**Batch Name :**").replace("[", " | ").replace("]", "").replace("- aiplex.lol", ".mp4\n\n**Downloaded By Using Bot Of** 𝐑𝐄𝐗𝐎𝐃𝐀𝐒 🇮🇳").replace("Downloaded by Bot of", "").replace("Love Babbar/Data Structures & Algorithms Master Course [Supreme Batch]/", "Love Babbar DSA Supreme Batch").replace("Downloaded by : 𝐒𝚝𝚞𝚋𝚋𝘰𝚛η❤️", "").replace("𝚂𝚝𝚞𝚋𝚋𝚘𝚛𝚗", "").replace("File No:", "").replace("File name:", "").replace("Batch Name : 𝐁𝐚𝐬𝐢𝐜 𝐓𝐨 𝐁𝐫𝐢𝐥𝐥𝐢𝐚𝐧𝐜𝐞", "").replace("Batch Name : 𝐆𝐫𝐚𝐦𝐦𝐚𝐫 𝐏𝐫𝐚𝐜𝐭𝐢𝐜𝐞 𝐏𝐫𝐨", "").replace("Batch Name : 𝐋𝐞𝐯𝐞𝐥 𝐔𝐩 𝐘𝐨𝐮𝐫 𝐄𝐧𝐠𝐥𝐢𝐬𝐡", "").replace("Batch Name : 𝐒𝐮𝐩𝐞𝐫 𝐂𝐡𝐚𝐫𝐠𝐞 𝐘𝐨𝐮𝐫 𝐕𝐨𝐜𝐚𝐛𝐮𝐥𝐚𝐫𝐲", "").replace(" ", ""),
                ...
                await client.send_document(
                    sender,
                    path, 
                    caption=caption.replace("@ImTgLoki", "**ＡＳＵＲ**\n`@AsurTalkBot`").replace("@LokiTheRobot", "**ＡＳＵＲ**\n`@AsurTalkBot`").replace("@TgLokii", "**ＡＳＵＲ**\n`@AsurTalkBot`").replace("by♛ऋषभ राजपूत","").replace("ℍ𝕖𝕝𝕡 𝕠𝕥𝕙𝕖𝕣𝕤 𝔾𝕠𝕕 𝕨𝕚𝕝𝕝 𝕙𝕖𝕝𝕡 𝕐𝕠𝕦", "").replace("RESOLUTION:-720", "").replace("Download By : 👑 KING R & 👸QUEEN P", "").replace("BATCH NAME :-" ,"**Batch Name :**").replace("[", " | ").replace("]", "").replace("- aiplex.lol", ".mp4\n\n**Downloaded By Using Bot Of** 𝐑𝐄𝐗𝐎𝐃𝐀𝐒 🇮🇳").replace("Downloaded by Bot of", "").replace("Love Babbar/Data Structures & Algorithms Master Course [Supreme Batch]/", "Love Babbar DSA Supreme Batch").replace("Batch: Youth Publication.json", "𝖩𝗈𝗂𝗇  ➥ 「 @PDFHUBREBORNNEW 」").replace("Vid_id", "**Pdf_Id**").replace("Downloaded by : 𝐒𝚝𝚞𝚋𝚋𝘰𝚛η❤️", "").replace("𝚂𝚝𝚞𝚋𝚋𝚘𝚛𝚗.pdf", "PDF").replace("File No:", "").replace("File name:", "").replace("Batch Name : 𝐁𝐚𝐬𝐢𝐜 𝐓𝐨 𝐁𝐫𝐢𝐥𝐥𝐢𝐚𝐧𝐜𝐞", "").replace("Batch Name : 𝐆𝐫𝐚𝐦𝐦𝐚𝐫 𝐏𝐫𝐚𝐜𝐭𝐢𝐜𝐞 𝐏𝐫𝐨", "").replace("Batch Name : 𝐋𝐞𝐯𝐞𝐥 𝐔𝐩 𝐘𝐨𝐮𝐫 𝐄𝐧𝐠𝐥𝐢𝐬𝐡", "").replace("Batch Name : 𝐒𝐮𝐩𝐞𝐫 𝐂𝐡𝐚𝐫𝐠𝐞 𝐘𝐨𝐮𝐫 𝐕𝐨𝐜𝐚𝐛𝐮𝐥𝐚𝐫𝐲", "").replace(" ", ""),
                    thumb=thumb_path,
                    progress=progress_for_pyrogram,
                    progress_args=(
                        client,
