           ...
           # medi =  msg.document or msg.video or msg.audio or None
            ...
            if str(file).split(".")[-1] in ['mkv', 'mp4', 'webm', 'mpe4', 'mpeg']:
                ...
                await client.send_video(
                    chat_id=sender,
                    video=path,
                    caption=caption.replace("@ImTgLoki", "**ＡＳＵＲ**\n`@AsurTalkBot`").replace("@LokiTheRobot", "**ＡＳＵＲ**\n`@AsurTalkBot`").replace("@TgLokii", "**ＡＳＵＲ**\n`@AsurTalkBot`").replace("by♛ऋषभ राजपूत","").replace("ℍ𝕖𝕝𝕡 𝕠𝕥𝕙𝕖𝕣𝕤 𝔾𝕠𝕕 𝕨𝕚𝕝𝕝 𝕙𝕖𝕝𝕡 𝕐𝕠𝕦", "").replace("RESOLUTION:-720", "").replace("Download By : 👑 KING R & 👸QUEEN P", "").replace("BATCH NAME :-" ,"**Batch Name :**").replace("[", " | ").replace("]", "").replace("- aiplex.lol", "").replace("Downloaded by Bot of", "").replace("Love Babbar/Data Structures & Algorithms Master Course [Supreme Batch]/", "Love Babbar DSA Supreme Batch"),
